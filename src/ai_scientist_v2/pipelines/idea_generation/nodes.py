...
class IdeaGenerationState(MessagesState):
    current_round: int
    last_tool_result: Optional[str]
    generated_idea: Optional[Dict[str, Union[str, List[str]]]]
...
def idea_generation_node(
    ...
    # logger.info("Starting idea generation node.")
    ...
    return {
        ...
        "generated_idea": None,  # No idea generated yet
    }
...
def idea_reflection_node(
    ...
    # logger.info(
    #     f"Starting idea reflection node ({state['current_round']} / {conf.max_reflections})."
    # )
...
def idea_finalization_node(
    ...
    output = agent_with_structured_output.invoke(messages)
    assert isinstance(output, Idea)
    ...
    return {
        ...
        "current_round": state["current_round"],
        "last_tool_result": last_tool_result,
        "generated_idea": output.model_dump(),
    }
